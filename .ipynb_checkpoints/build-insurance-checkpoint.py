...
def create_database():
    ...
    # Load avg insurance 2015
    try:
        print("\nLoading average insurance 2015 dataset...")           
        url = 'https://raw.githubusercontent.com/djolly09/Psych_755_Final/refs/heads/main/data/average_insurance_2015.csv'
        insurance_2015_data = pd.read_csv(url,index_col = 0)            
        insurance_2015_data.to_sql('insurance_2015', conn, index = False) 
        print(f"✅ Successfully loaded insurance 2015 dataset with {len(insurance_2015_data)} rows")
    except Exception as e:
        print(f"❌ Error loading insurance 2015 dataset: {str(e)}")
